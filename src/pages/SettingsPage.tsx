...
import { ApartmentOutlined, AppstoreOutlined, RobotOutlined } from '@ant-design/icons';
...
import DingTalkPage from './DingTalkPage';
...
const SUBMENU = [
  ...
  { key: 'dingtalk', label: '外接钉钉', icon: <RobotOutlined /> },
];
...
function renderSub(sub?: string) {
  ...
  if (sub === 'dingtalk') return <DingTalkPage />;
...
export default function SettingsPage() {
  ...
  const { sub } = useParams();
  const active =
    sub && (sub === 'branches' || sub === 'projects' || sub === 'general' || sub === 'dingtalk')
      ? sub
      : 'branches';
