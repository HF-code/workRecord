...
- **构建/MR 按轨**：每轨独立目标环境（显式值或按"下一环境"推导），构建/MR 作用于
  该需求全部有效项目（排除「不参与构建」）。
...
- `src/types.ts`：双轨字段（envWeizan/envStar/testPass*/target*），删 STATUSES/PipelineType/SortMode/currentEnv/buildEnv。
...
- `src/hooks/useWorkTracker.ts`：删排序体系（reorder/sortByReleaseDate/moveToPublishedTop）；`migrateToDualTrack`；`useBuildPlan(update)` 按轨（getTarget/getBuildOther/setTarget）。
- `src/batch.ts`：collectMrTargets/collectBuildTargets 按「已开始轨」展开（每轨独立目标环境）。
- `src/components/RequirementForm.tsx`：去状态/流水线/目标分支，恢复版本 Select。
- `src/components/RequirementCard.tsx`：双轨块（状态行：轨点+派生状态+测试通过；操作行：阶段/目标/构建/MR），去拖拽与子勾选。
...
- `src/components/BatchPanel.tsx`：按轨展示目标环境与清单。
