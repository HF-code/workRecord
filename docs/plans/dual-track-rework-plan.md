...
1. **「不参与构建」恢复生效——仅约束构建**：卡片构建 / 批量构建重新按 `excludeFromBuild` 跳过项目
   （如 `vzanlive_weapp` 小程序不单独构建）；**提交 MR 不受影响**——卡片提交MR / 批量MR 仍覆盖需求全部项目
   （MR 只生成预填链接，代码都要合）。即第三轮第 2 条「构建/MR 全量化」修正为「仅 MR 全量化」。
2. 实现落点：`batch.ts` 新增 `isBuildExcluded()` / `getBuildItems()` / `collectBuildExcludedProjects()`；
   `getBatchItems()` 语义收窄为 **MR / 展示口径**（仅临时排除），并改名 `getMrItems()`
   以防复用（构建/`getBuildItems`、MR/`getMrItems` 两条链路禁止合并）；
   `collectBuildTargets()` / `summarize()` 增 `apps` 入参以读取配置。
3. 不再恢复卡片的「不构建」灰显标识：改为
   - 批量面板统计行加「N 个项目不参与构建」提示（Tooltip 列项目名）；
   - 批量面板逐需求小框的项目 Tag 对不构建项目标注「（不构建）」并降透明度（仍可 X 临时排除，MR 仍用它）；
   - 单卡构建 / 批量构建完成后的 message 明示「已跳过不参与构建的项目：xxx」。
4. 快速构建页逻辑不变（默认跳过 + 「本次包含」开关，本就是构建场景）。
