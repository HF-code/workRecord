...
**它消灭的痛点**：以前一个需求上线，要手点运维平台给 N 个项目各构建一次（运维平台手动逐个构建），并自己盯制品好没好。现在：一次点击 → N 个项目并行构建 → 遇"上一任务未完成"自动重试 → 制品好了自动回填链接。
...
| 前端函数（`build.ts`） | 用途 |
|---|---|
| `requestBuild` | 单步提交构建：`POST /deploy/build`，构建编号由响应体返回 |
...
| 位置 | 名称 | 作用 |
|---|---|---|
| L24/L32 | `BuildPlanLike` / `BuildPlan` | 入参类型（只需 req + 分支配置） |
| L35/L50/L57 | `MrTarget` / `MrSkipped` / `BuildTarget` | 输出项：待做 MR / 被跳过 / 待构建 |
| L72 | `BatchSummary` | 汇总结果（构建项 + MR 项 + 排除统计） |
| L88 | `getMrItems()` | **MR / 展示口径**：一个需求的全部项目（仅临时排除） |
| L98 | `isBuildExcluded()` | 单项目是否被配置"不参与构建"；**只允许构建链路与纯展示调用** |
| L106 | `getBuildItems()` | **构建口径**：全部项目 − 临时排除 − 配置「不参与构建」 |
| L118 | `collectBuildExcludedProjects()` | 被配置排除出构建范围的项目名（提示文案用） |
| L140 | `collectMrTargets()` | 按已开始轨展开 (需求 × 目标环境 × 项目) |
| L191 | `collectBuildTargets()` | 按 `project::buildOther` **去重合并**成构建项（入参含 `apps`） |
| L234 | `summarize()` | 一次性算全部（面板用） |
| L255 | `getCardTone()` | 整体状态 → 卡片背景色 |

**关键点**：
1. `collectBuildTargets` 的 key 是 `项目::构建命令`，所以**同项目同分支只构建一次**，但不同分支会各构建一次。
2. **构建与 MR 的口径差异只有一处**：配置「不参与构建」（`excludeFromBuild`）对构建生效（走 `getBuildItems`），
   对 MR 不生效（走 `getMrItems`）——MR 只生成预填链接，代码都要合。
3. ⚠️ **构建与 MR 的范围函数禁止合并或互相复用**（历史踩坑：曾共用 `getBatchItems` →
   「不参与构建」连坐 MR；后又为修 MR 把过滤整体删掉，反噬构建）。要增删范围条件，
   只改自己那条链路的函数。
...
| L176 | `startBuildTask()` | **入口**：提交构建 → 失败分支（重试/报错） |
...
| L169 | `handleTrackBuild` | 单轨构建：`getBuildItems`（剔除「不参与构建」）→ 逐项目 `startBuildTask` |
| L228 | `openMrTabs` | **MR 统一入口**：超阈值先二次确认 → `openManyTabs` 全部打开 → 如实提示 |
| L260 | `handleTrackMr` | 单轨 MR：`getMrItems`（全量，不受「不参与构建」影响）→ 拼链接 → `openMrTabs` |
| L288 | `handleToggleSelect` | 卡片勾选 |
| L310/L331 | `handleRemoveItem` / `handleClearSelection` | 批量面板的临时排除/清空（**会话态，不落盘**） |
| L337 | `handleBatchMr` | 批量 MR（`collectMrTargets` → `openMrTabs` + 跳过清单） |
| L351 | `handleBatchBuild` | 批量构建（`collectBuildTargets` → 并发 `startBuildTask`） |
| L402 起 | `openCreateForm` / `handleSubmit` / `handleDelete` / `handleExportAll` / `handleMigrateLegacy` / `handleExportAndClean` / `handleImportFile` | 增删改 + 导入导出；**一键迁移旧数据** = 自动备份 + 就地转换（不走文件往返） |
...
| 改"不参与构建"语义 | `batch.ts` `isBuildExcluded()` / `getBuildItems()`（构建）/ `getMrItems()`（MR） | 开关在 `ProjectConfigPage`，字段是 `DevopsApp.excludeFromBuild`；**构建受它约束，MR 不受**；两个范围函数禁止合并 |
...
| 旧数据 / `legacy-raw` | 旧版格式的需求数据；一键迁移只在其上做**就地替换**（不走导入的按 id 合并） | `utils/legacyImport.ts` `migrateLegacyList` |
| 构建命令 / buildOther | 运维平台 `build_other` 参数 | `config/branches.ts` `buildOther` |
| 构建编号 / number | 构建提交响应体返回的序号 | `build.ts` `requestBuild` |
...
| 批量范围（构建） | 勾选卡片 × 全部项目 − 临时排除 − 配置排除 | `batch.ts` `getBuildItems` / `collectBuildTargets` |
| 批量范围（MR） | 勾选卡片 × 全部项目 − 临时排除（配置排除不生效） | `batch.ts` `getMrItems` / `collectMrTargets` |
| 临时排除/恢复 | 会话内操作，刷新即失效 | `RequirementListPage` 的 `batchExcluded` |
| 不参与构建 | 项目级持久化开关；**只约束构建，不影响 MR** | `DevopsApp.excludeFromBuild` |
