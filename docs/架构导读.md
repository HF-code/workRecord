...
> **基准版本**：2026-09-16（`src/` 36 个文件 / 5,338 行）。行号会随改动漂移，**请优先用「函数名 / 常量名」定位**。
...
| `hooks/useWorkTracker.ts` | 216 | 需求/项目/分支读写 + 一次性迁移 | ★★ |
...
| `storage.ts` | 157 | localStorage 读写 + 旧数据迁移 | ★★ |
| `components/RequirementForm.tsx` | 150 | 需求新增/编辑表单 | ★★ |
| `components/ArtifactList.tsx` | 132 | 制品清单（跨页复用的同一份数据） | ★ |
| `hooks/useBuildNotifications.tsx` | 128 | 全局构建完成通知（弹窗/系统通知/标题） | ★ |
| `export.ts` | 125 | 导入导出 + 数据校验 | ★★ |
...
| `utils/openTabs.ts` | 24 | 逐个打开 MR 链接 | ✗ |
...
| 键 | 内容 |
|---|---|
| `work-tracker:requirements:v1` | 全部需求（唯一事实源） |
...
| `work-tracker:build-envs:v1`、`work-tracker:build-selected:v1` | **旧版遗留**，迁移时读取后弃用 |
...
flowchart LR
    R["需求 Requirement"]
    R --> W["微赞轨<br/>envWeizan / testPassWeizan / targetWeizan"]
...
| 字段 | 含义 |
|---|---|
| `env<轨>` | 当前阶段。`undefined`=轨被移除，`null`=未开始，否则为环境名 |
| `testPass<轨>` | 手动"测试通过"标记。**不改变阶段**，只影响卡片文案与推导 |
| `target<轨>` | 构建/MR 的目标环境。有值则用之，无值则按当前阶段推导"下一环境" |
...
| `trackTargetOf()` | 取该轨实际目标环境（显式 target 优先，否则 `defaultTrackTarget` 推下一环境） |
| `isTrackOnline()` | 该轨是否已走到末段（master / prod-txnj） |
| `startedTracks()` | 该需求已开始的轨（`env !== undefined` 才算存在） |
| `backfillRequirement()` | 补全老数据缺失字段（迁移用） |
...
| L136 | `defaultTrackTarget()` | 缺省目标环境 = 当前阶段的下一环境 |
| L145 | `trackTargetOf()` | 实际目标环境（显式 target 优先） |
| L152 | `hasExplicitTarget()` | **当前无业务引用，属暂留代码**，别照抄 |
| L160 | `LEGACY_STATUS_TO_ENV` | 旧状态枚举 → 环境名（迁移映射） |
| L188 | `backfillRequirement()` | 老数据补字段 |
...
| L124 | `handleAdvanceTrack` | 推进阶段（**会重置该轨测试通过标记**） |
| L137 | `handleToggleTestPass` | 勾选/取消测试通过（不动阶段） |
| L142 | `handleSetTarget` | 设置该轨目标环境（显式 target 的来源） |
| L147 | `handleRemoveTrack` | 移除整条轨（置 `undefined`） |
| L158 | `handleTrackBuild` | 单轨构建：`getBatchItems` → 逐项目 `startBuildTask` |
| L201 | `handleTrackMr` | 单轨 MR：拼链接 → 逐个新标签页打开 |
| L233 | `handleToggleSelect` | 卡片勾选 |
| L261/L282/L290 | `handleRemoveItem` / `handleIncludeItem` / `handleClearSelection` | 批量面板的临时排除/恢复/清空（**会话态，不落盘**） |
| L297 | `handleBatchMr` | 批量 MR（`collectMrTargets` → 打开链接 + 生成跳过清单） |
| L308 | `handleBatchBuild` | 批量构建（`collectBuildTargets` → 并发 `startBuildTask`） |
| L353-L440 | `openCreateForm` / `handleSubmit` / `handleDelete` / `handleExportAll` / `handleExportAndClean` / `handleImportFile` | 增删改 + 导入导出 |
...
| 入口 | 目标环境 | 去重 | 备注 |
|---|---|---|---|
| 卡片构建 | 该轨 `target`（缺省推导） | 不去重（该需求全部项目） | 一对一，最直接 |
| 批量构建 | 每需求每已开始轨各自 target | `项目::构建命令` 合并 | 一次覆盖多需求 |
...
| 我想改… | 落点 | 连带 / 坑 |
|---|---|---|
| 需求加一个字段（如"优先级"） | `types.ts` `Requirement` → `RequirementForm` 加表单项 → `RequirementCard` 渲染 | 若参与导入导出，补 `export.ts` 的 `isValidRequirement`；若参与筛选，补 `FilterBar` + `RequirementListPage` 的 `filtered` |
| 新增版本标签类型 | `types.ts` `VERSIONS` | 表单下拉自动生效（组件遍历该常量） |
| 项目条目加字段 | `types.ts` `ProjectBranch` → `RequirementForm` → `batch.ts` | 老数据缺字段要在 `backfillRequirement` 补默认值 |
| 兼容老数据新字段 | `storage.ts` 加 `migrateXxx()` + 在 `useWorkTracker` 的 `useState` 初始化里调用 | 必须幂等 + try/catch；见第八章坑 4 |

### B. 领域规则

| 我想改… | 落点 | 连带 / 坑 |
|---|---|---|
| 环境链路增删/调整顺序 | `config/track.ts` `TRACK_ENVS` | 同时改 `ENV_CLUSTER`、`CLUSTER_COLOR`、`LEGACY_STATUS_TO_ENV`（老数据映射）、`config/branches.ts` 的 `DEFAULT_BRANCHES` |
...
| 改导入导出格式 | `export.ts` | 导入有校验 `isValidRequirement`，注意向后兼容 |
...
4. **迁移只跑一次**：`storage.ts` 的 `migrate*` 靠标记键防重放（如 `LEGACY_ENVS_KEY`、迁移标记）。**修了迁移 bug 后老用户不会自动重跑**——需要换新标记键或写补偿逻辑。
...
| 目标环境 / target | 构建/MR 的落点，缺省=下一环境 | `track.ts` `trackTargetOf` |
| 显式目标 | 用户手动指定的 target | `track.ts` `hasExplicitTarget` |
